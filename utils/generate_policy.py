...
def generate_policy_docx(doc, data):
    ...
    if data["liability"]["selected"]:
        replace_in_all_paragraphs(doc, "$XXXX/人", data['liability']['bi_per_person'])
        replace_in_all_paragraphs(doc, "$XXX", data['liability']['bi_per_accident'], occurrence=1)
        replace_in_all_paragraphs(doc, "$XXXX", data['liability']['pd'], occurrence=1)
    else:
...
def replace_in_all_paragraphs(doc, keyword, replacement, occurrence=None):
    count = 0
    for paragraph in doc.paragraphs:
        for run in paragraph.runs:
